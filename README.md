...
The game loads the `assets/` and `data/` folders beside the executable. CMake copies both folders after building. `build.ps1` builds the DX11 executable and Jolt-backed smoke test in `build-jolt-ninja/`, then copies the executable to the project root. Jolt is pinned to v5.6.0 under `third_party/JoltPhysics/`; its MIT license is included in packages. To regenerate baked meshes and skinning clips after editing a source GLB, run `python tools/convert_assets.py` with Python 3. The converter uses only the Python standard library. DX11 binds an adjacent `.png`, `.jpg`, or `.jpeg` base-color texture for each known baked static mesh; embedded normal/roughness model maps and full PBR glTF materials remain future pipeline work.

Run `./package.ps1` to build the DX11 game, copy its executable with the complete `assets/` and `data/` directories and Jolt license, smoke-test day, night, causeway, and each distant biome from the copied folder, and create a zip under `dist/`. Use `./package.ps1 -SkipBuild` to package the latest existing build. `-IncludeOpenGL` includes the existing fallback executable without rebuilding it.
