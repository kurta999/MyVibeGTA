...
struct Ped{Vec2 p,target;float speed,angle,respawn=0;bool alive=true;Color shirt;int health=100;float panic=0;int style=0;bool armed=false,hostile=false;float fireCooldown=0,hitFlash=0;
    ...
    float grievance=0,pursuitMemory=0,carSearchCooldown=0,boardingTime=0;
...
struct Vehicle{Vec2 p;float angle=0,speed=0;Kind kind;Color c;
    ...
    int driver=-1,reservedBy=-1,roadFrom=-1,roadTo=-1,routeChoice=0;
    TrafficState trafficState=TrafficState::Parked;
    float reactionTime=0,blockedTime=0,desiredSpeed=0;
};
...
struct ShellCasing{Vec3 p,v;float life=0,rotation=0,spin=0;};
...
extern std::vector<ShellCasing> casings;
...
void damageVehicle(int index,float amount,bool playerCaused=false);
